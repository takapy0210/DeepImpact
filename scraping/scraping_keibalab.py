...
with open('race_database-keibalab.csv', 'w') as csvfile:
				...
				if sum:
					...
					if hr.find(u'のレース結果') > 0:
						...
						for row in rows:
							td = row.find_all('td')
							if len(td) >= 15:
								horse_urls.append(td[3].find('a').get("href"))
								hr = eval('b"{}"'.format(str(td[3].find('a').string))).decode('UTF8')
								jk = eval('b"{}"'.format(str(td[6].find('a').string))).decode('UTF8')
								odds = str(td[8].find('span').string).strip().replace('\\t', '').replace('\\n', '')
								nink = str(td[7].string)
